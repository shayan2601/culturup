    ...
    <div className="bg-gray-50 min-h-screen py-4 sm:py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-4 sm:mb-6">
          <a href="#" className="inline-flex items-center text-gray-600 hover:text-gray-900 text-sm sm:text-base">
        ...
        <div className="lg:grid lg:grid-cols-3 lg:gap-8">
          ...
          <div className="lg:col-span-2 mb-6 lg:mb-0">
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              ...
              />
              <div className="p-3 sm:p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center border-t border-gray-100 gap-3 sm:gap-0">
                <div className="flex space-x-4">
                  <button 
                    className="flex items-center text-gray-500 hover:text-red-500 transition-colors"
                    onClick={() => setIsLiked(!isLiked)}
                  >
                    <Heart className={`h-4 w-4 sm:h-5 sm:w-5 mr-1 ${isLiked ? 'fill-red-500 text-red-500' : ''}`} />
                    <span className="text-sm sm:text-base">{isLiked ? artwork.stats.likes + 1 : artwork.stats.likes}</span>
                  </button>
                  <div className="flex items-center text-gray-500">
                    <Eye className="h-4 w-4 sm:h-5 sm:w-5 mr-1" />
                    <span className="text-sm sm:text-base">{artwork.stats.views}</span>
                  </div>
                </div>
                <div className="flex space-x-2">
                  ...
                  >
                    <Bookmark className={`h-4 w-4 sm:h-5 sm:w-5 ${isBookmarked ? 'fill-cyan-600 text-cyan-600' : 'text-gray-500'}`} />
                  </button>
          ...
          <div className="lg:col-span-1">
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
              <div className="flex flex-col sm:flex-row justify-between items-start mb-4 gap-3 sm:gap-0">
                <div className="flex-1">
                  <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">{artwork.title}</h1>
                ...
                <div className="bg-cyan-50 text-cyan-700 px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap">
                  {artwork.category}
                </div>
              </div>
              ...
              <div className="border-t border-b border-gray-100 py-4 mb-6">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-baseline mb-4 gap-2 sm:gap-0">
                  <span className="text-2xl sm:text-3xl font-bold text-gray-900">{formatPrice(currentBid)}</span>
                  <span className="sm:ml-2 text-sm text-gray-500">({bidHistory.length} bids)</span>
                </div>

                {/* Bid input - Fixed responsiveness */}
                <div className="mb-4">
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-0">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        className="pl-8 pr-4 py-3 w-full rounded-lg sm:rounded-r-none sm:rounded-l-lg border border-gray-300 focus:ring-cyan-500 focus:border-cyan-500 text-sm sm:text-base"
                        placeholder={`Min. ${formatPrice(currentBid + artwork.auction.minBidIncrement)}`}
                      />
                    </div>
                    <button
                      onClick={handleBidSubmit}
                      className="bg-cyan-600 hover:bg-cyan-700 text-white font-medium px-4 py-3 rounded-lg sm:rounded-l-none sm:rounded-r-lg transition-colors duration-200 text-sm sm:text-base whitespace-nowrap"
                    >
                      Place Bid
                    </button>
                  </div>
              ...
              <button 
                ...
                className="w-full py-2 px-4 bg-gray-100 hover:bg-gray-200 rounded text-gray-700 text-sm font-medium flex justify-center items-center mb-6 transition-colors"
              >
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bidder</th>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {bidHistory.map((bid, index) => (
                          <tr key={index}>
                            <td className="px-3 sm:px-4 py-3 whitespace-nowrap text-sm text-gray-900 truncate max-w-0">{bid.user}</td>
                            <td className="px-3 sm:px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{formatPrice(bid.amount)}</td>
                            <td className="px-3 sm:px-4 py-3 whitespace-nowrap text-sm text-gray-500">{bid.time}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div className="space-y-3">
                ...
                <p className="text-sm text-gray-600 flex items-start">
                  ...
                  <span>{artwork.details.certificate}</span>
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
              ...
              <div className="flex items-center mb-3">
                <div className="rounded-full bg-gray-200 overflow-hidden h-10 w-10 sm:h-12 sm:w-12 mr-3 flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center">
                    <h4 className="font-medium text-gray-900 truncate">{artwork.artist.name}</h4>
                    {artwork.artist.verified && (
                      <CheckCircle className="h-4 w-4 text-cyan-600 ml-1 flex-shrink-0" />
                    )}
                  </div>
                  <p className="text-sm text-gray-600">{artwork.artist.artworks} artworks</p>
                </div>
              </div>
              <p className="text-sm text-gray-600 mb-3 leading-relaxed">{artwork.artist.bio}</p>
            ...
            {similarArtworks.map((item, index) => (
              <div key={index} className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-200">
                <div className="relative">
                  <img src="https://images.unsplash.com/photo-1499892477393-f675706cbe6e?w=900&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MjB8fHBhaW50c3xlbnwwfHwwfHx8MA%3D%3D" alt={item.title} className="w-full h-40 sm:h-48 object-cover" />
                  <div className="absolute top-2 sm:top-3 right-2 sm:right-3 bg-white rounded-full p-1.5 shadow-md">
                    <Bookmark className="h-3 w-3 sm:h-4 sm:w-4 text-gray-600" />
                  </div>
                </div>
                <div className="p-3 sm:p-4">
                  <h3 className="text-base sm:text-lg font-bold text-gray-900 truncate">{item.title}</h3>
                  <p className="text-sm text-gray-600 truncate">by {item.artist}</p>
                  <p className="mt-2 text-cyan-600 font-bold text-sm sm:text-base">{item.price}</p>
                </div>
              </div>
            ))}
          </div>
